...
#define dbg_waring(module, fmt,...) dbg_print(module, DBG_LEVEL_WARNING, __FILE__, __FUNCTION__, __LINE__, fmt, ##__VA_ARGS__)
...
#define dbg_assert(expr,msg) { \
    if(!(expr))                    \
    {                              \
        dbg_print(DBG_LEVEL_INFO,DBG_LEVEL_ERROR, __FILE__, __FUNCTION__, __LINE__, "assert failed : "#expr", msg"); \
        while(1){}                     \
}}
